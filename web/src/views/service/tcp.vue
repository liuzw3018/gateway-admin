          ...
          <el-form ref="form" :model="form" label-width="130px">
            ...
            <el-form-item label="主机白名单">
              <el-input
                ...
                type="textarea"
                autosize
                placeholder="格式：hostname 多条换行，白名单优先于黑名单"
              />
            </el-form-item>
            ...
            <el-form-item label="禁用IP列表">
              <el-input
                v-model="form.forbid_list"
                type="textarea"
                autosize
...
<script>
import { addTCPService, getServiceDetail, updateTCPService } from '@/api/service'
...
export default {
  ...
  methods: {
    ...
    handleCreate(row, index) {
      ...
      if (this.isEdit === true) {
        updateTCPService(createQuery).then(response => {
        ...
        addTCPService(createQuery).then(response => {
          this.submitDisable = false
          this.$notify({
            title: 'Success',
            message: '添加成功',
