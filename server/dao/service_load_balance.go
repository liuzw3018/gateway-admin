...
/**
 * @Author: liu zw
 * @Date: 2022/10/18 12:22
 * @File: service_load_balance.go
 * @Description: //TODO $
 * @Version:
 */
...
func (lb *LoadBalance) Find(c *gin.Context, tx *gorm.DB, search *LoadBalance) (*LoadBalance, error) {
	out := &LoadBalance{}
...
func (lb *LoadBalance) Save(c *gin.Context, tx *gorm.DB) error {
	return tx.WithContext(c).Save(lb).Error
}

func (lb *LoadBalance) GetIPListByModel(c *gin.Context, tx *gorm.DB) []string {
